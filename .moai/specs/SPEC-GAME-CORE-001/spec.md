...
- An admin UI or CRUD API for editing squad numbers in the Supabase player-data table beyond the manual entry process (REQ-SYNC-004)
- Bulk-import tooling beyond the manual, per-player entry mechanism — squad number is maintained one player at a time, keyed by player id, not via a seed-and-replace dataset like the Korean mapping table
